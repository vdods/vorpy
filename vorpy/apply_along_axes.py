...
def apply_along_axes (func, input_axis_v, input_array, *args, output_axis_v=None, func_output_shape=None, **kwargs):
    ...
    **Note that, just as input_axis_v, output_axis_v may contain values in the range [-N,0), in which
    ...
    assert is_subsequence_of_nondecreasing_sequence(normalized_input_axis_v, A), 'normalized_input_axis_v (which is {0}) must be a nonempty subsequence of [0,{1}) (right endpoint excluded)'.format(normalized_input_axis_v, len(input_array.shape))
